...
`npm run build:desktop` 会先准备独立 Node.js 运行时，再生成 Windows x64 安装版和便携版。v0.6.1 自动化验证为 **52/52 通过**，覆盖 DSH 启动、项目登记、余额密钥隔离、Codex 式新对话与最近记录、应用内文件分组编辑、分组检索、更新下载与哈希校验、解析顺序、连续文本块合并、表格、坐标、三种问答模式、多轮上下文、正文/文后原文链接、引用定位和标准接口。

更多文档：[安装说明](docs/INSTALL_CN.md) · [更新机制](docs/UPDATES_CN.md) · [v0.6.1 发布说明](docs/RELEASE_NOTES_0.6.1_CN.md)
